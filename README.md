# 💻 PabloG.Dev

> Site profissional para desenvolvedor front-end com portfólio, serviços e suporte multilíngue (PT/ES/EN).

🌐 **Website:** https://www.pablogdev.com.br  

---

## 🎯 Sobre o Projeto

O **PabloG.Dev** é o meu site profissional - a vitrine do meu trabalho como desenvolvedor front-end. Aqui apresento meus serviços, portfólio com projetos reais, processo de trabalho e um assistente virtual para facilitar o contato.

Construído com **React 19**, **TypeScript**, **Vite**, **GSAP** e **Framer Motion**, o site reflete minha abordagem profissional: performance, animações fluidas, design impecável e experiência internacional com suporte multilíngue.

---

## 🚀 Tecnologias

<div align="center">

![React](https://img.shields.io/badge/React-19-61DAFB?style=flat-square&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-6.0-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-8.0-646CFF?style=flat-square&logo=vite&logoColor=white)
![GSAP](https://img.shields.io/badge/GSAP-3.15-88CE02?style=flat-square&logo=greensock&logoColor=white)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-12.38-0055FF?style=flat-square&logo=framer&logoColor=white)
![Lenis](https://img.shields.io/badge/Lenis-1.3-000000?style=flat-square)

</div>

- **React 19** - Biblioteca para construção de interfaces
- **TypeScript 6.0** - Tipagem estática
- **Vite 8.0** - Build tool rápido
- **GSAP 3.15** - Animações profissionais
- **Framer Motion** - Animações e transições
- **Lenis** - Scroll suave e performance
- **Lucide React** - Ícones vetoriais
- **React Icons** - Ícones de redes sociais
- **Vite PWA** - Progressive Web App

---

## ⚡ Funcionalidades

### 🌍 Multilíngue
- Suporte a 3 idiomas: Português, Espanhol, Inglês
- URLs amigáveis (/es, /en, /)
- Bandeiras no header e menu mobile

### 🏠 Hero Section
- Imagem de fundo com efeito
- Título com destaque dourado
- Features: WhatsApp, Agendamentos, Gestão, 24/7
- Botões: WhatsApp e Diagnóstico Grátis

### 🛠️ Serviços
- Sites que passam confiança
- Sistemas para organizar seu negócio
- Agendamentos online
- Benefícios listados por serviço

### 🖼️ Showcase (Portfólio)
- 12+ projetos reais
- Categorias: Eventos, E-commerce, Alimentação, Saúde, Imobiliário, Beleza, Bem-Estar
- Hover com overlay de detalhes
- Links diretos para cada projeto

### 📋 Processo de Trabalho
- 6 passos: Conversa → Planejamento → Design → Desenvolvimento → Revisão → Entrega
- Timeline visual com números e ícones

### ❓ FAQ
- 6 perguntas frequentes
- Accordion com animação
- CTA para WhatsApp

### 📞 Contato
- WhatsApp e Email
- Redes sociais
- Mensagem automática personalizada

### 💬 Floating AI
- Assistente virtual integrado

---

## 🎨 Destaques do Design

- **Paleta Escura** - #000c24 com destaque dourado #fec90f
- **Tipografia** - Inter (sans-serif) + Playfair Display (serif)
- **Animações GSAP** - Transições profissionais
- **Scroll Suave com Lenis** - Experiência premium
- **Menu Mobile** - Com animação suave
- **Seletor de Idioma** - Bandeiras no header e mobile
- **Cards com Hover** - Overlay detalhado no showcase
- **Floating AI** - Assistente virtual

---

## 📱 Responsividade

- **Mobile** - Otimizado para smartphones
- **Tablet** - Layout adaptado para telas médias
- **Desktop** - Experiência completa

---

## 💼 Sobre este Projeto

Este é o meu site profissional - a minha presença digital como desenvolvedor front-end. Aqui mostro quem sou, o que faço e os resultados que entrego. Cada detalhe foi pensado para transmitir profissionalismo, qualidade e confiança.

---

<div align="center">

### Desenvolvido por Pablo Gomes

[![Website](https://img.shields.io/badge/Website-PabloG.Dev-8bc34a?style=flat-square)](https://www.pablogdev.com.br)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat-square&logo=github&logoColor=white)](https://github.com/pablog-7)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/pablogomesdev)

</div>
